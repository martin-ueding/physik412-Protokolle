...
Hier geben wir einen $\pi/2$ Puls und nach einer Verzögerungszeit $\tau$ noch
einen weiteren $\pi/2$-Puls. Die Magnetisierung kippt durch den ersten Puls in
...
kippen wir um weitere $\pi/2$ und messen dann das Signal.

Für sehr kleine Verzögerungszeiten $\tau$ erwarten wir kein Signal, da die
Magnetisierung dann um $\pi$ gekippt immer noch parallel zur $z$-Achse ist.
...
$\pi/2$-Puls. Für sehr kurze Zeiten $\tau$ entspricht dies einem
...
Betrachten wir ein einzelnes FID-Signal nach einem $\pi/2$-Puls, so fällt dies
