...
Die davon sichtbaren sind: \SIlist{
%<- for lambda in balmer_berechnet: ->%
<< lambda >>;
%<- endfor ->%
}{\nano\meter}.
...
Die Daten, die wir mit dem PC aufgenommen haben, stellen wir graphisch dar. Die
erste rote Linie (Abbildung~\ref{fig:rot}) konnten wir gut abbilden und die
Isotopieaufspaltung beobachten. 

An einen ausgewählten Bereich passen wir eine Gaußfunktion an:
\[
    I(\alpha) = a \exp\del{- \frac{(\alpha - \alpha_0)^2}{2 \sigma}}
\]

Die Halbwertsbreite $H$ ist gegeben
als:\footnote{\url{https://en.wikipedia.org/wiki/Gaussian_function} 2013-10-30
17:32}
\[
...
\begin{figure}[htbp]
    ...
    \caption{%
        Graph der Daten zur ersten roten Linie. Es wurde ein konstantes
        Untergrundsignal von \num{<< red_underground >>} abgezogen. An die
        markierten Datenpunkte wurde jeweils eine Gaussfunktion angepasst. Der
        Mittelwert des Hauptmaximums liegt bei \SI{<< red_primary_center
        >>}{\degree} und hat eine Halbwertsbreite von \SI{<< red_primary_fwhm
        >>}{\degree}. Das Nebenmaximum liegt bei \SI{<< red_secondary_center
        >>}{\degree} und hat eine Halbwertsbreite von \SI{<< red_secondary_fwhm
        >>}{\degree}.
    }
    \label{fig:rot}
\end{figure}
...
}{\bibliographyfile existiert nicht.}
