...
\begin{abstract}
    ...
    diesem die Energie eines Niveau-Übergangs von Quecksilber.
\end{abstract}
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        % Brechungsindizes
        \node[label=180:{$n>1$}, label=0:{$n_\text{Luft}=1$}] at (1,3.2) {};
        % Lot
        \draw[dashed]
        (-2.5,2.) -- (0.5,2.)
        (-0.5,-3.) -- (2.5,-3.);
        ...
        \draw[dotted] (c) -- (2., -2.) node[left]{.} ;
        \draw (1.75,-1.75) arc(135:225:0.35);
    \end{tikzpicture}
    \caption{%
...
\begin{figure}
    ...
    \begin{tikzpicture}[thick]
        ...
        \draw
            ...
            (4, -0.72) -- (4, 2.22)
            (4, 2.3) circle(0.08) node[left] {\small{$+$}}
            (4, 2.7) circle(0.08) node[left] {\small{$-$}}
            (4, 2.78) -- (4,3.2)
            (4, 3.5) circle(.3) node {A}
            (4, 3.8) -- (4, 4)
        ;
        \draw[fill]
            (4,-4) circle(0.05)
        ...
        \node[right] at (4, 2.5) {$U_\text{Gegen}$};
...
Durch eine angelegte Heizspannung treten Elektronen aus der Glühkathode aus.
Diese werden dann durch die Beschleunigungsspannung in Richtung Anode
Beschleunigt. Auf dem Weg stoßen sie mit den Quecksilberatomen, sie verlieren
aber nur dann ihre Energie, wenn diese genau einem Übergang im Termschema des
Quecksilbers entspricht. Nach einem solchen inelastischen Stoß wird das
Elektron wieder beschleunigt. Da die Anode netzförmig ist, fliegen die meisten
Elektronen durch diese hindurch zur Gegenkathode. Sie können dort als Strom
gemessen werden.

Wird nun die Beschleunigungsspannung langsam von \SI{0}{\volt} an erhöht, steigt
...
Zur Betrachtung der Polarisation kann zwischen den Kondensor und das Etalon
ein Linearpolarisator und ein $\frac{\lambda}4$-Plättchen gestellt werden.
Für die Kalibrierung des Magnetfeldes wird die Cadmiumlampe durch eine
Hall-Sonde ersetzt.

Eine Skizze dieses Aufbaus ist in Abbildung~\ref{fig:Aufbau_Zeeman} zu sehen.
...
\begin{figure}[htpb]
    ...
    \begin{tikzpicture}[thick]
        ...
        (.8,0)--(.8,.8)
        (0,.5) rectangle (.5,1.5)
        (.5,.5) coordinate (vru) -- (1.5,1.5) coordinate (hru)
        (.5,1.5) coordinate (vro) -- (1.5,2.5) coordinate (hro)
        (0,1.5) coordinate (vlo) -- (1,2.5) coordinate (hlo)
        -- (hro) -- (hru)
        ($(vlo)!1/3!(hlo)$) -- ($(vro)!1/3!(hro)$) -- ($(vru)!1/3!(hru)$)
        ($(vlo)!2/3!(hlo)$) -- ($(vro)!2/3!(hro)$) -- ($(vru)!2/3!(hru)$)
        (.25,1) circle (.05)
        ;
        \draw[->, >=latex, shorten >= -2pt, thin]
        (.65,.4) arc (120:330: .2 and .1);
        % erste Linse
        \draw
        (3,0) -- (3,.75)
        -- (2.9,.75) -- (2.9,0)
        (3,1.5) ellipse (.3 and .75)
        (3,1.5) ellipse (.2 and 0.6)
        (2.9,.75) arc (270:90: .3 and .75)
        (3,2.25) -- (2.9,2.25)
        ;
        % Etalon
        \draw
        (5,0) -- (5,.75)
        -- (4.9,.75) -- (4.9,0)
        (5,1.5) ellipse (.3 and .75)
        (5,1.5) ellipse (.15 and 0.5)
        (4.9,.75) arc (270:90: .3 and .75)
        (5,2.25) -- (4.9,2.25)
        ;
        % zweite Linse
        \draw
        (7,0) -- (7,.75)
        -- (6.9,.75) -- (6.9,0)
        (7,1.5) ellipse (.3 and .75)
        (7,1.5) ellipse (.2 and 0.6)
        (6.9,.75) arc (270:90: .3 and .75)
        (7,2.25) -- (6.9,2.25)
        ;
        % Interferenzfilter
        \draw
        (8,0) -- (8,.75)
        -- (7.9,.75) -- (7.9,0)
        (8,1.5) ellipse (.3 and .75)
        (8,1.5) ellipse (.15 and 0.5)
        (7.9,.75) arc (270:90: .3 and .75)
        (8,2.25) -- (7.9,2.25)
        ;
        % Okular
        \draw
        (9,0) -- (9,.75)
        -- (8.9,.75) -- (8.9,0)
        (9,.75) arc (270:24: .3 and .75)
        (9,.75) arc (270:336: .3 and .75)
        (8.9,.75) arc (270:90: .3 and .75)
        (9,2.25) -- (8.9,2.25)
        (9.5,1.2) -- (9,1.2) arc (270:90: .08 and .3) -- (9.5,1.8)
        (9.5,1.5) ellipse (.08 and .3)
        (9.5,1.5) ellipse (.04 and .2)
        ;
        % Beschriftungen
        % Lampe und Magnet
        \draw
        ...
        (1.1, 2.3) --(.7,3)
        ;
        \node[above] at (.65, 3) {\parbox{2cm}{\center{\small{Magnet-\\
        polschuhe}}}};
        \draw
        (1.0,1.5) -- (1.3,0.8) node[right]
        {\parbox{1.2cm}{\center{\small{Cd-\\Lampe}}}};
        % erste Linse
        \draw (2.9,2.3) -- (2.9,3) node[above]
        {\parbox{2cm}{\center{\small{Linse\\$f =
        \SI{150}{\milli\meter}$}}}};
        % Etalon
        \draw (4.9,2.3) -- (4.9,3) node[above]
        {\parbox{2cm}{\center{\small{Fabry-Pérot-\\Etalon}}}};
        % zweite Linse
        \draw (6.8,2.3) -- (6.5,2.6) node[above]
        {\parbox{2cm}{\center{\small{Linse\\$f =
        \SI{150}{\milli\meter}$}}}};
        % Interferenzfilter
        \draw (7.9,2.3) -- (7.9,3) node[above]
        {\parbox{1.8cm}{\center{\small{Interferenz-\\filter}}}};
        % Okular
        \draw (9.35,1.9) -- (9.4,2.5) node[above] {\small{Okular}};

        % Stromquelle
        \draw
        (-1,1) -- ++ (-1,0) -- ++ (0,1) -- ++ (.5,.5) -- ++ (1,0) -- ++ (-.5,-.5)
        coordinate (cube) -- ++ (-1,0)
        (cube) -- ++ (0,-1) -- ++ (.5,.5) -- ++ (0,1)
        (-1.5,1.5) circle (.2) node[label=330:{\small{$I$}}]{};
        \draw[->]
        (-1.75,1.25) to (-1.25,1.75);
        \draw[out=0, in=180]
        (-.75,1.6) to (0,1)
        (-.75,1.7) to (0,1.1)
        (-.75,1.9) to (.6,2.1)
        (-.75,1.8) to (.5,2);
        \node[below] at (-1.5,1) {\parbox{2cm}{\small{Stromquelle\\(regelbar)}}};
    \end{tikzpicture}
    \caption{%
        Versuchsaufbau zur Beobachtung des Zeeman-Effektes.
    }
...
können (Abbildung~\ref{fig:Skizze-2}). Der Strom, bei dem die Linien zu
...
Nun fahren wir wieder den Magnetstrom hoch, sodass wir die Linien trennen
...
wählen das erste Maximum auf der Seite der negativen Winkel aus.
...
Die Einträge in der Fehlermatrix sind um mehrere Größenordnungen größer
...
Die Maxima sind aus den Daten der CCD Zeile zu unterscheiden, wenn sie einen
