...
\begin{table}[htbp]
    ...
    \begin{tabular}{SSSSS}
        {$I / \si{\ampere}$}
        & {$B / \si{\centi\tesla}$}
        & {$\sigma_- / \si{\pico\meter}$}
        & {$\sigma_0 / \si{\pico\meter}$}
        & {$\sigma_+ / \si{\pico\meter}$} 
        \\
        \midrule
        %<- for row in sigma_tabelle: ->%
        << ' & '.join(row) >> \\
        %<- endfor ->%
    \end{tabular}
    \caption{%
        Breite der Maxima, angegeben ist die Standardabweichung der Gaußkurve.
    }
    \label{tab:Peakbreiten}
\end{table}
...
\FloatBarrier
\subsection{Wellenzahlunterschiede}
...
benutzen:
\[
    \Deltaup k = \frac{U_{\text{Abstand}} e}{h c}.
\]
Die erhaltenen Wellenzahlen sind in Tabelle~\ref{tab:fh-k}.
...
\begin{figure}[htbp]
    ...
    \caption{%
        \ccbyncsa \parencite{Krähling/Termschema_Quecksilber}
    }
    \label{fig:}
\end{figure}
